...
export async function scheduleGdprSlackCheck(intervalMinutes: number = 60): Promise<void> {
...
async function checkGdprDeadlines(): Promise<void> {
  ...
  const pendingRequests = await db.query.gdprRequests.findMany({
    where: and(eq(gdprRequests.status, "pending"), lte(gdprRequests.createdAt, twentyDaysAgo)),
    with: {
      contact: true,
    },
  });
