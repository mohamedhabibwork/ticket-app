...
import { ArrowLeft, Save, GripVertical, Trash2, Settings, Eye, Plus, X } from "lucide-react";
...
function FormBuilderRoute() {
  ...
  const formDataFromState = location.state?.formData as { name: string; description: string; formType: string } | undefined;
  ...
  const renderFieldPreview = (field: FormField) => {
    switch (field.type) {
      ...
      case "number":
        return (
          <Input type={field.type === "email" ? "email" : field.type === "phone" ? "tel" : "text"} placeholder={field.placeholder} />
        );
      case "textarea":
              ...
              {fields.length === 0 ? (
                <div className="h-full flex items-center justify-center">
                  <div className="text-center">
                    <p className="text-muted-foreground mb-2">Drag fields from the left panel to build your form</p>
                  ...
                  {fields.map((field, index) => (
                    <div
                      ...
                      className={`relative group bg-background rounded-none border-2 transition-colors ${
                        selectedFieldId === field.id ? "border-primary" : "border-border hover:border-muted-foreground/30"
                      }`}
                      ...
                      <div className="flex items-start gap-3 p-4">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-2">
                            <span className="text-xs bg-muted px-2 py-0.5 rounded-none">{field.type}</span>
                            <Label className="text-sm font-medium">{field.label}</Label>
                            {field.required && <span className="text-xs text-destructive">*</span>}
                          </div>
                          {renderFieldPreview({ ...field, placeholder: field.placeholder || field.label })}
                          {field.helpText && (
                ...
                <div className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) => setFieldSettings({ ...fieldSettings, label: e.target.value })}
                    />
                  </div>
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) => setFieldSettings({ ...fieldSettings, placeholder: e.target.value })}
                    />
                  </div>
                  <div className="space-y-2">
                    ...
                    <Textarea
                      ...
                      onChange={(e) => setFieldSettings({ ...fieldSettings, helpText: e.target.value })}
                      rows={2}
                    />
                  </div>
                  {["radio", "select", "multi-select"].includes(selectedField.type) && (
                    <div className="space-y-2">
                      ...
                      <Textarea
                        ...
                        onChange={(e) => setFieldSettings({ ...fieldSettings, options: e.target.value })}
                        rows={4}
                  ...
                  <div className="flex items-center gap-2">
                    <Checkbox
                      ...
                      onCheckedChange={(checked) => setFieldSettings({ ...fieldSettings, required: checked as boolean })}
                    />
